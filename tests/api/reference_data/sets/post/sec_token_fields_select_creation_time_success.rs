...
#[tokio::test]
pub(crate) async fn post_reference_set_with_sec_token_fields_select_creation_time_success() {
    ...
    let response_body: CreationTime = serde_json::from_slice(&response_body_bytes)
    ...
    assert_eq!(
        ...
        CreationTime {
            ...
            // as we only test that the returned value is valid u128.
            // The assert_eq! here is only to present the expected structure
            creation_time: 0
        }
    );
}
