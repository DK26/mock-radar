...
#[tokio::test]
pub(crate) async fn post_reference_set_with_sec_token_fields_multiple_select_timeout_type_and_name_success(
) {
    ...
    assert_eq!(
        response_body,
        json!({"timeout_type": timeout_type, "name": name})
    );
}
